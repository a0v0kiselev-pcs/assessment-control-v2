...
public final class PredefinedStepDefs {
    @Given("^I open url \"([^\"]*)\"$")
    ...
    private static final Duration waitForTimeout = Duration.ofSeconds(10);
    private static final Duration waitForPolling = Duration.ofMillis(200);

    @Then("^I wait for element with xpath \"([^\"]*)\" to be present$")
    public void iWaitForElementWithXpathToBePresent(final String xpath) {
        new WebDriverWait(getDriver(), waitForTimeout, waitForPolling)
                .until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpath)));
    }

    @Then("^I wait for element with xpath \"([^\"]*)\" to not be present$")
    public void iWaitForElementWithXpathToNotBePresent(final String xpath) {
        new WebDriverWait(getDriver(), waitForTimeout, waitForPolling)
